...
const initPayload = () => {
  ...
  parent_id.value = prop.data?.parent_id
...
const updateData = async () => {
  waiting.value = true
  try {
    await axiosIns.patch(`/job-levels/${id.value}`, {...payload})
    close(false, true)
  } catch (error) {
    console.log(error);
    ...
    <VForm ref="formEdit" @submit.prevent="onSubmit()">
      <VCard>
        ...
        <VCardText>
          <VRow>
            <VCol cols="4">
              <VTextField label="Parent ID" v-model="parent_id" />
            </VCol>
            <VCol cols="8">
