...
const fetchData = async () => {
    waiting.value = true;
    try {
        const ret = await axiosIns.get('/attendance-locations', {
            params: { ...args },
        })
...
const deleteData = async id => {
  waiting.value = true
  try {
    await axiosIns.delete(`/attendance-locations/${id}`)
    await fetchData()
  } catch (error) {
    console.log(error)
...
const importExcelHandler = async (event) => {
  ...
  try {
    const response = await axiosIns.post('/attendance-locations/import', formData, {
      headers: {
    ...
    <VCard>
      <VCardText>
        <VRow class="d-flex flex-wrap align-center">
          ...
          <VCol class="d-flex flex-wrap gap-2 align-center">
            <VTextField placeholder="Find .." v-model="searchQuery" clearable @keyup.enter="findHandler"
              @click:clear="clearHandler" prepend-inner-icon="tabler:search" style="flex: 1 1 auto;" />
            <VBtn variant="outlined" prepend-icon="tabler-plus" @click="addFormVisible = true">Add</VBtn>
              <div>
              <input 
