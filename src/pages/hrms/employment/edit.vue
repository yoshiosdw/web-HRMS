...
const employee_identity_number = ref()
...
const initPayload = () => {
  ...
  employee_id.value = prop.data?.employee_id
  employee_identity_number.value = prop.data?.employee?.identity_number || null
  organization_id.value = prop.data?.organization?.id || null
  job_level.value = prop.data?.level_job?.id || null
  job_position.value = prop.data?.position_job?.id || null
  from_date.value = prop.data?.from_date
  to_date.value = prop.data?.to_date
  is_active.value = prop.data?.is_active
}
...
const updateData = async () => {
  ...
  try {
    ...
    await axiosIns.patch(`/employments/${id.value}`, { ...payload })
    close(false, true)
  } catch (error) {
    console.error(error)
...
const getEmployee = val => {
  ...
  employee_identity_number.value = val.identity_number
}
        ...
        <VCardText>
          <VRow>
            <VCol cols="6" class="d-flex gap-3">
              <VTextField
                label="Employee"
                v-model="employee_identity_number"
                readonly
