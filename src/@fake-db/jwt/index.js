...
mock.onPost('/auth/login').reply(request => {
  ...
  if (user) {
    try {
      const tkiAccessToken = userTokens[user.id]

      // We are duplicating user here
      const tkiUserData = { ...user }

      const userOutData = Object.fromEntries(Object.entries(tkiUserData)
        .filter(([key, _]) => !(key === 'password' || key === 'abilities')))

      const response = {
        tkiUserAbilities: tkiUserData.abilities,
        tkiAccessToken,
        tkiUserData: userOutData,
      }


      //   const tkiAccessToken = jwt.sign({ id: user.id }, jwtSecret)
      return [200, response]
    }
    catch (e) {
      errors = { email: [e] }
...
mock.onPost('/auth/register').reply(request => {
  ...
  if (!errors.username && !errors.email) {
    ...
    console.log('tkiUserData :>> ', tkiUserData)
    database.push(tkiUserData)

    const tkiAccessToken = userTokens[tkiUserData.id]
    const { password: _, abilities, ...user } = tkiUserData

    const response = {
      tkiUserData: user,
      tkiAccessToken,
      tkiUserAbilities: abilities,
    }
